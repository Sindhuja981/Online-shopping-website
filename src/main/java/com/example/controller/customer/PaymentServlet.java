	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//check the given card number, cvv, expirydate, cardtype is correct or not
				try {
				    ...
				    String qry ="select * from bankaccounts where cardNumber=? and cardType=? and cvvNumber=? and exipryDate=?";
				    ...
				    if(rs.next()) {  //account exist
				    	...
				    	if(totalFinalAmount>rs.getInt("balanceAmount")) { //not enough blanace in a/c
				    		pw.println("<script language='javascript'>alert('Not enough balance in A/C. Transction failed'); window.location='CustomerHome.jsp';</script>");
				    		return;
				    		...
				    		pst1.executeUpdate();
				    		System.out.println("qry1 for update in bankAccount="+pst1.toString());
				    		...
				    		pst2.executeUpdate();
				    		System.out.println("qry2 for update in bankAccount="+pst2.toString());
				    		...
				    		pst3.executeUpdate();
				    		System.out.println("qry3="+pst.toString());
		...
		String dateFormate = sdf.format(cal.getTime());
		System.out.println("ddd=" + dateFormate);
		//create today
		...
		// Add order details , payment details
				...
				for (product productOb : ListProducts) {
					long invoiceNumber = Long.parseLong(dateFormate+i);
					Long orderId = Long.parseLong(dateFormate+i);
		...
		// Add order details
			...
			//System.out.println("paymentType:" + paymentType);
				  //Empty cart.
					...
					//Send invoice mail
					SendMail.sendMail(session.getAttribute("customerEmailId").toString(), "Invoice orders", content);
					
					response.sendRedirect("/ECommerceProject/admin/OrdersHistory.jsp");
